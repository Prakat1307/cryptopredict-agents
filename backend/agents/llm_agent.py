...
class LLMAgent(BaseAgent):
    ...
    async def run(self, task: str='analyze', context: Dict[str, Any]=None, **kwargs) -> Dict[str, Any]:
        ...
        if not self.available:
            return {'reasoning': 'LLM reasoning not available (no API key)', 'recommendation': 'Use statistical predictions only', 'timestamp': timestamp_now()}
        try:
            if task == 'analyze':
            ...
            self.add_to_memory({'action': f'llm_{task}', 'context': context, 'result': result})
            self.status = 'idle'
            return result
        except Exception as e:
            self.status = 'error'
            self.logger.log_error(e, {'task': task, 'context': context})
            return {'reasoning': f'LLM error: {str(e)}', 'recommendation': 'Fallback to statistical analysis', 'timestamp': timestamp_now()}
    ...
    async def _analyze_market(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {'analysis': response, 'asset': asset, 'timestamp': timestamp_now(), 'model_used': self.model}
    ...
    async def _explain_prediction(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = await self._call_llm(prompt)
        return {'explanation': response, 'timestamp': timestamp_now()}
    ...
    async def _arbitrage_reasoning(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = await self._call_llm(prompt)
        return {'arbitrage_analysis': response, 'asset': asset, 'timestamp': timestamp_now()}
    ...
    async def _risk_assessment_reasoning(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = await self._call_llm(prompt)
        return {'risk_advice': response, 'timestamp': timestamp_now()}

    async def _call_llm(self, prompt: str, model: Optional[str]=None) -> str:
        use_model = model or self.model
        headers = {'Authorization': f'Bearer {self.api_key}', 'Content-Type': 'application/json', 'HTTP-Referer': 'https://cryptopredict-agents.local', 'X-Title': 'CryptoPredict Agents'}
        payload = {'model': use_model, 'messages': [{'role': 'system', 'content': 'You are a concise crypto trading analyst. Provide brief, actionable insights.'}, {'role': 'user', 'content': prompt}], 'max_tokens': self.max_tokens, 'temperature': self.temperature}
        try:
            response = await self.http_client.post(f'{self.base_url}/chat/completions', headers=headers, json=payload, timeout=30.0)
