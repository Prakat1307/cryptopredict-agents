"""
Agent Orchestrator - Coordinates all agents in the CryptoPredict system.
Implements the Hermes Agent flow with feedback loops.

Pipeline:
1. SearchAgent -> finds markets on Polymarket & Kalshi
2. DataAgent -> fetches OHLCV via Apify/Binance (1000 bars)
3. PredictionAgent -> Kronos model + statistical fallback
4. LLMAgent -> OpenRouter reasoning & analysis
5. RiskAgent -> Kelly Criterion position sizing
6. FeedbackAgent -> Hermes learning loop
"""
...
class AgentOrchestrator:
    """
    Orchestrates the multi-agent pipeline with Hermes Agent pattern.
    
    Agents:
    1. SearchAgent - Polymarket + Kalshi market discovery
    2. DataAgent - Apify + Binance OHLCV fetch (1000 bars)
    3. PredictionAgent - Kronos foundation model (github.com/shiyu-coder/Kronos)
    4. LLMAgent - OpenRouter LLM reasoning (meta-llama/llama-3.1-8b-instruct:free)
    5. RiskAgent - Kelly Criterion position sizing
    6. FeedbackAgent - Hermes Agent learning loop
    """
    ...
    async def initialize(self):
        ...
        await asyncio.gather(
            ...
            self.llm_agent.initialize(),
            self.risk_agent.initialize(),
            self.feedback_agent.initialize()
        )
    ...
    async def shutdown(self):
        ...
        await asyncio.gather(
            self.search_agent.shutdown(),
            self.data_agent.shutdown(),
            self.prediction_agent.shutdown(),
            self.llm_agent.shutdown(),
            self.risk_agent.shutdown(),
            self.feedback_agent.shutdown()
        )
    ...
    async def run_prediction_pipeline(
        ...
        """
        Run the full prediction pipeline for an asset.
        
        Pipeline Steps:
        1. Search markets on Polymarket & Kalshi
        2. Fetch 1000 bars of OHLCV data via Apify/Binance
        3. Predict using Kronos foundation model
        4. LLM reasoning & market analysis (OpenRouter)
        5. Kelly Criterion risk assessment
        6. Hermes feedback loop storage
        """
        self.logger.info(f"Running prediction pipeline for {asset}/{timeframe}")
        
        pipeline_start = datetime.utcnow()
        
        # Step 1: Search markets on Polymarket & Kalshi
        market_data = None
        ...
        # Step 2: Fetch historical data (1000 bars via Apify, fallback to Binance)
        ...
        # Step 3: Generate prediction using Kronos
        ...
        # Step 4: LLM reasoning (OpenRouter)
        llm_analysis = None
        if use_llm:
            try:
                llm_analysis = await self.llm_agent.run(
                    task="analyze",
                    context={
                        "asset": asset,
                        "prediction": prediction,
                        "market_data": {
                            "polymarket_price": self._extract_polymarket_price(market_data),
        ...
        # Step 5: Risk assessment with Kelly Criterion
        risk = await self.risk_agent.run(
            prediction=prediction,
            asset=asset
        )
        
        # Step 6: Hermes feedback loop
        ...
        result = {
            ...
            "recommended_position": risk["position_size"],
            "polymarket_price": poly_price,
            "kalshi_price": kalshi_price,
            "timestamp": timestamp_now(),
            "reasoning": self._generate_reasoning(prediction, risk, llm_analysis),
            "pipeline_latency_ms": round(pipeline_latency * 1000, 2),
            "risk_level": risk["risk_metrics"]["risk_level"],
            "regime": prediction["regime"],
            "llm_analysis": llm_analysis,
            "kronos_info": prediction.get("kronos_info"),
            "model_used": prediction.get("model_used", "statistical")
        }
        ...
        self.logger.info(
            f"Pipeline complete: {asset}/{timeframe} -> {prediction['direction'].upper()} "
            f"(confidence: {prediction['confidence']:.2f}, kelly: {risk['kelly_fraction']:.4f})"
        )
    ...
    async def check_arbitrage(self, asset: str, timeframes: List[str] = None) -> List[Dict[str, Any]]:
        ...
        Example: 15min UP prediction vs next 3x 5min UP predictions.
        If 15min says UP but 5min sequence doesn't align, there's an edge.
        """
        ...
        for tf in timeframes:
            try:
                pred = await self.run_prediction_pipeline(
                    asset=asset, 
                    timeframe=tf,
        ...
        # Look for inconsistencies (arbitrage signals)
        ...
        for i in range(len(tfs)):
            for j in range(i + 1, len(tfs)):
                ...
                # Check for direction mismatch with high confidence
                    ...
                    if p1["confidence"] > 0.55 and p2["confidence"] > 0.55:
                        ...
                        if spread > min_spread:
                            opp = {
                                ...
                                "timeframe_a": tf1,
                                "timeframe_b": tf2,
                                "prediction_a": p1["prediction"],
                                "prediction_b": p2["prediction"],
                                "confidence_a": p1["confidence"],
                                "confidence_b": p2["confidence"],
                                "upside_prob_a": p1["upside_probability"],
                                "upside_prob_b": p2["upside_probability"],
                                "spread": round(spread, 4),
                                "recommended": f"trust_{tf1}" if p1["confidence"] > p2["confidence"] else f"trust_{tf2}",
                                "timestamp": timestamp_now()
                            }
                            opportunities.append(opp)
                
                # Check for same direction but different confidence (edge sizing)
                    ...
                    if conf_diff > 0.15:
                        opp = {
                            ...
                            "recommended": f"size_by_{tf1}" if p1["confidence"] > p2["confidence"] else f"size_by_{tf2}",
                            "timestamp": timestamp_now()
                        }
                        opportunities.append(opp)
        
        # Internal arbitrage: 15min vs next 3x 5min
        if "15m" in predictions and "5m" in predictions:
            p_15m = predictions["15m"]
            p_5m = predictions["5m"]
            
            # If 15min says UP but 5min says DOWN, the 5min might reverse
            if p_15m["prediction"] == "up" and p_5m["prediction"] == "down":
                opportunities.append({
                    "type": "internal_arbitrage_15m_vs_5m",
                    "asset": asset,
                    ...
                    "edge": round(p_15m["confidence"] - p_5m["confidence"], 4),
                    "timestamp": timestamp_now()
                })
            
            # If both agree with high confidence, strong signal
            if p_15m["prediction"] == p_5m["prediction"] and p_15m["confidence"] > 0.65 and p_5m["confidence"] > 0.65:
                opportunities.append({
                    "type": "aligned_signal",
                    "asset": asset,
                    "description": f"Both 15m and 5m agree on {p_15m['prediction'].upper()} with high confidence",
                    "strategy": "Aggressive position sizing",
                    "combined_confidence": round((p_15m["confidence"] + p_5m["confidence"]) / 2, 4),
                    "timestamp": timestamp_now()
                })
        
        # Add LLM arbitrage reasoning if available
        if opportunities and self.llm_agent.available:
            try:
                llm_arb = await self.llm_agent.run(
                    task="arbitrage",
                    context={"opportunities": opportunities, "asset": asset}
                )
    ...
    async def run_multi_asset_cycle(self, assets: List[str] = None, timeframes: List[str] = None):
        """
        Run prediction cycle for multiple assets (scaling feature).
        Default: BTC, ETH + additional assets from config.
        """
        if assets is None:
            assets = self.assets
        if timeframes is None:
    ...
    async def _check_cross_asset_arbitrage(self, predictions: List[Dict]) -> List[Dict]:
        """Check for arbitrage opportunities between different assets."""
        opportunities = []
        ...
        # Look for divergent predictions on same timeframe
        for tf, preds in by_tf.items():
            ...
            if ups and downs:
                # Some assets up, some down - potential pairs trade
                opportunities.append({
                    "type": "cross_asset_divergence",
                    "timeframe": tf,
                    "up_assets": [p["asset"] for p in ups],
                    "down_assets": [p["asset"] for p in downs],
                    "strategy": "Long strongest UP asset, short strongest DOWN asset",
                    "timestamp": timestamp_now()
                })
    ...
    def get_status(self) -> Dict[str, Any]:
        """Get overall system status with all agents."""
        return {
            "status": "running",
            "agents": {
                "search": self.search_agent.get_status(),
                "data": self.data_agent.get_status(),
                "prediction": self.prediction_agent.get_status(),
                "llm_reasoning": self.llm_agent.get_status(),
                "risk": self.risk_agent.get_status(),
                "feedback": self.feedback_agent.get_status()
            },
            "assets_tracked": self.assets,
            "scaling": {
                "multi_asset": self.scaling_config.get("multi_asset", {}).get("enabled", False),
                "arbitrage": self.scaling_config.get("arbitrage", {}).get("enabled", False),
                "parallel_execution": self.scaling_config.get("parallel_execution", {}).get("enabled", False)
            },
            "last_update": timestamp_now(),
            "active_predictions": len(self.active_predictions),
            "total_predictions": len(self.prediction_history.get_all()),
    ...
    def get_prediction_history(self, asset: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get prediction history, optionally filtered by asset."""
        if asset:
    ...
    def _extract_polymarket_price(self, market_data: Any) -> Optional[float]:
        """Extract Polymarket price from search results."""
    ...
    def _extract_kalshi_price(self, market_data: Any) -> Optional[float]:
        """Extract Kalshi price from search results."""
        if not market_data or not isinstance(market_data, dict):
            return None
        ...
        if kal and kal.get("markets"):
            ...
            if yes_bid is not None:
                try:
                    return float(yes_bid) / 100  # Kalshi prices are in cents
                except (ValueError, TypeError):
                    pass
        return None
    
    def _generate_reasoning(self, prediction: Dict, risk: Dict, llm_analysis: Any) -> str:
        """Generate human-readable reasoning combining all agents."""
        parts = []
    ...
    def _calculate_overall_accuracy(self) -> Optional[float]:
        """Calculate overall prediction accuracy from feedback."""
