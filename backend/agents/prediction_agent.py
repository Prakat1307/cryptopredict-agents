...
class PredictionAgent(BaseAgent):
    ...
    async def initialize(self):
        ...
        try:
            ...
            if loaded:
                self.logger.logger.info(f'Kronos model loaded successfully from HuggingFace: {self.model_name}')
            else:
                self.logger.logger.warning('Kronos model not available - using statistical fallback. To use Kronos, ensure git and transformers are installed.')
        except Exception as e:
            self.kronos_available = False
            self.logger.logger.warning(f'Could not load Kronos model: {e}. Using statistical fallback.')
    ...
    async def run(self, asset: str='BTC', data: pd.DataFrame=None, timeframe: str='5m', **kwargs) -> Dict[str, Any]:
        ...
        self.logger.log_decision(f'Predicting {asset} for {timeframe} timeframe', {'asset': asset, 'data_points': len(data), 'timeframe': timeframe})
        try:
            pred_config = self.pred_config.get('pred_lengths', {})
            pred_len = pred_config.get('short', 5) if timeframe in ['1m', '5m'] else pred_config.get('medium', 15) if timeframe in ['15m', '30m'] else pred_config.get('long', 60)
            ...
            result = {'asset': asset, 'timeframe': timeframe, 'prediction_length': pred_len, 'timestamp': timestamp_now(), 'direction': prediction['direction'], 'confidence': prediction['confidence'], 'upside_probability': prediction['upside_probability'], 'expected_return': prediction['expected_return'], 'volatility_forecast': prediction['volatility'], 'regime': prediction.get('regime', 'unknown'), 'monte_carlo': mc_results, 'model_used': prediction.get('model', 'statistical'), 'kronos_info': self.kronos.get_model_info() if self.kronos_available else None}
            self.add_to_memory({'action': 'predict', 'asset': asset, 'prediction': result})
            self.status = 'idle'
            return result
        except Exception as e:
            self.status = 'error'
    ...
    async def _kronos_predict(self, data: pd.DataFrame, pred_len: int) -> Dict[str, Any]:
        try:
            result = self.kronos.predict(df=data, pred_len=pred_len, sample_count=self.sample_count, temperature=1.0)
            result['regime'] = detect_regime(data['close'])
            result['last_price'] = round(data['close'].iloc[-1], 2)
            self.logger.log_action('kronos_predict', {'direction': result['direction'], 'confidence': result['confidence'], 'model': result.get('model', 'kronos')})
            return result
        except Exception as e:
    ...
    async def _statistical_predict(self, data: pd.DataFrame, pred_len: int) -> Dict[str, Any]:
        ...
        combined_score = weights['trend'] * trend_score + weights['rsi'] * rsi_score + weights['momentum'] * momentum_score
        upside_prob = 1 / (1 + np.exp(-combined_score * 3))
        ...
        return {'direction': direction, 'confidence': round(confidence, 4), 'upside_probability': round(upside_prob, 4), 'expected_return': round(expected_return, 6), 'volatility': round(vol, 6), 'regime': regime, 'last_price': round(last_price, 2), 'model': 'statistical', 'signals': {'trend': trend, 'rsi': round(rsi, 2), 'momentum': round(momentum_score, 4)}}
    ...
    async def _ensemble_predict(self, predictions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {'direction': direction, 'confidence': round(confidence, 4), 'upside_probability': round(ensemble_prob, 4), 'ensemble_size': len(predictions)}
    ...
    async def _monte_carlo_simulate(self, data: pd.DataFrame, pred_len: int, n_sims: int=100) -> Dict[str, Any]:
        ...
        return {'upside_probability': round(mc_upside_prob, 4), 'mean_final_price': round(np.mean(final_prices), 2), 'std_final_price': round(np.std(final_prices), 2), 'percentile_5': round(np.percentile(final_prices, 5), 2), 'percentile_95': round(np.percentile(final_prices, 95), 2), 'n_simulations': n_sims, 'sample_paths': paths[:5, :min(10, pred_len + 1)].tolist()}