"""
Risk Management Agent - Uses Kelly Criterion for position sizing.
Implements advanced risk management with portfolio correlation analysis.
"""
...
class RiskAgent(BaseAgent):
    ...
    Implements Kelly Criterion, portfolio optimization, and drawdown controls.
    """
    ...
    async def run(self, prediction: Dict[str, Any], asset: str = "BTC", **kwargs) -> Dict[str, Any]:
        """
        Run risk management for a prediction.
        
        Args:
            prediction: Prediction result from PredictionAgent
            asset: Asset symbol
        
        Returns:
            Dict with position sizing and risk metrics
        """
        self.status = "running"
        self.last_run = timestamp_now()
        
        self.logger.log_decision(
            f"Assessing risk for {asset} prediction",
        ...
        try:
            ...
            result = {
                ...
                "recommendation": self._generate_recommendation(kelly, risk, dd_control),
                "position_size": kelly["recommended_size"],
                ...
                "max_position": self.bankroll * self.diversification_max
            }
    ...
    async def _kelly_size(self, prediction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # For prediction markets, odds are implied by probability
        # If probability is p, fair odds are (1-p)/p
        p = upside_prob * confidence  # Adjusted probability
        q = 1 - p
        
        # Expected payoff ratio (simplified)
        # In binary markets, you win 1 unit or lose your stake
        # But market prices are not exactly 0.5, so we adjust
        if p > 0.5:
            b = p / q  # Odds in favor
        else:
            b = q / p
        ...
        # Recommended: half Kelly for safety
        recommended = min(half_kelly, self.bankroll * self.diversification_max)
        
        self.logger.log_action("kelly_calc", {
            "p": round(p, 4),
            "q": round(q, 4),
            "b": round(b, 4),
    ...
    async def _portfolio_optimize(self, assets_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        return {
            "weights": {k: round(v, 4) for k, v in zip(returns_dict.keys(), weights)},
            "expected_return": round(portfolio_return, 6),
            "volatility": round(portfolio_vol, 6),
            "sharpe": round(portfolio_return / portfolio_vol if portfolio_vol > 0 else 0, 4)
        }
    ...
    async def _risk_assess(self, asset: str, prediction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "risk_level": level,
            "confidence_risk": round((1 - confidence) * 0.3, 4),
            "volatility_risk": round(min(volatility * 10, 0.3), 4),
            "regime_risk": regime_risk.get(regime, 0.3),
            "recommendation": "proceed" if risk_score < 0.5 else "caution" if risk_score < 0.7 else "avoid"
        }
    
    async def _drawdown_control(self) -> Dict[str, Any]:
        """Monitor and control drawdown."""
        ...
        return {
            ...
            "total_pnl": round(float(cumulative[-1]) if len(cumulative) > 0 else 0, 2)
        }
