...
class FeedbackAgent(BaseAgent):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.model_weights = {'trend': 0.33, 'rsi': 0.33, 'momentum': 0.34}
    ...
    async def run(self, prediction: Dict[str, Any], actual_outcome: Optional[str]=None, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if actual_outcome:
                eval_result = await self.execute_tool('evaluate_prediction', prediction=prediction, actual=actual_outcome)
            ...
            result = {'timestamp': timestamp_now(), 'prediction_id': prediction.get('timestamp'), 'feedback': feedback, 'current_accuracy': await self.execute_tool('get_accuracy'), 'model_weights': self.model_weights.copy()}
            self.add_to_memory({'action': 'feedback', 'prediction': prediction, 'feedback': feedback})
            self.status = 'idle'
            return result
    ...
    async def _evaluate_prediction(self, prediction: Dict[str, Any], actual: str) -> Dict[str, Any]:
        ...
        result = {'prediction_id': prediction.get('timestamp'), 'predicted': predicted_direction, 'actual': actual, 'correct': correct, 'confidence': confidence, 'brier_score': round(brier_score, 4), 'log_loss': round(log_loss, 4), 'timestamp': timestamp_now()}
        ...
        if key not in self.performance:
            self.performance[key] = {'total': 0, 'correct': 0, 'brier_scores': [], 'confidences': []}
    ...
    async def _update_weights(self, evaluation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {'weights': self.model_weights, 'learning_rate': self.learning_rate, 'updated': True, 'recent_accuracy': round(recent_accuracy, 4)}
    ...
    async def _generate_feedback(self) -> Dict[str, Any]:
        ...
        for key, perf in self.performance.items():
            if perf['total'] > 5:
                ...
                info = {'key': key, 'accuracy': round(acc, 4), 'samples': perf['total'], 'avg_brier': round(avg_brier, 4)}
                if acc > 0.55:
        ...
        return {'overall_accuracy': accuracy, 'total_predictions': len(self.predictions.get_all()), 'total_evaluated': len(self.outcomes.get_all()), 'best_performers': best_performers[:5], 'worst_performers': worst_performers[:5], 'recommendations': self._generate_recommendations(best_performers, worst_performers, accuracy)}
    ...
    async def _get_accuracy(self) -> Dict[str, Any]:
        ...
        return {'accuracy': round(accuracy, 4), 'samples': total, 'brier_score': round(avg_brier, 4), 'calibration': round(calibration, 4), 'high_conf_accuracy': round(high_conf_correct / high_conf_total, 4) if high_conf_total > 0 else None}
    ...
    def get_stats(self) -> Dict[str, Any]:
        return {'predictions_stored': len(self.predictions.get_all()), 'outcomes_stored': len(self.outcomes.get_all()), 'performance_by_pair': self.performance, 'current_weights': self.model_weights, 'learning_rate': self.learning_rate}